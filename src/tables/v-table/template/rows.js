module.exports = function (h, that) {
  ...
  data.map(function (row, index) {
    ...
    if (that.opts.childRow) {
      columns.push(<td><span on-click={that.toggleChildRow.bind(that, row[rowKey])}
                             class={`VueTables__child-row-toggler ` + that.childRowTogglerClass(row[rowKey])}></span>
      </td>)
    }
    ...
    rows.push(<tr class={rowClass} on-click={that.rowWasClicked.bind(that, row)}
                  on-dblclick={that.rowWasClicked.bind(that, row)}>{columns} </tr>)
    ...
    if (that.opts.childRow && this.rowsToggleState['row_' + row[rowKey]]) {
      ...
      rows.push(<tr class='VueTables__child-row'>
        <td colspan={that.allColumns.length + 1}>{template}</td>
      </tr>)
    }
