<template>
  <doc-section id="popover" name="Popover">
    <div class="bs-example">
      <popover v-for="place in placements" :key="place.id" effect="fade" :content="text" :placement="place">
        <button class="btn btn-default">Popover on {{place}}</button>
      </popover>
      <hr>
      <h4>Title</h4>
      <popover v-for="place in placements" :key="place.id" effect="fade" header title="Title" :content="text" :placement="place">
        <button class="btn btn-default">Popover on {{place}}</button>
      </popover>
      <hr>
      <h4>Trigger</h4>
...
import popover from '../../src/components/bootue/popover/Popover.vue'
